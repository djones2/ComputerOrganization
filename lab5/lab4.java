	...
	public static void main(String[] args){
		...
		if (args.length > 0) {
			try {
				...
				mips = new MipsCPU(assembly, labels);			
				if (args.length > 1)
					script = args[1];
			...
			while (inScan.hasNextLine()) {
				...
				if (firstChar.compareTo("h") == 0) {
					...
					try {
						...
						int cycles = mips.getCycles();
						double CPI = (double)cycles / (double)mips.instructionsExecuted;
						
						System.out.println();
						System.out.println("Program complete");
						System.out.printf("CPI = %.3f Cycles = %d Instructions = %d\n",
							CPI, cycles, mips.instructionsExecuted);
						System.out.println();
					}
